...
- Template files (`AGENTS.template.md`, `REGISTRY.template.md`, `PRODUCT.template.md`, `CONSTITUTION.template.md`) are never forked per mode — all monorepo rendering logic (per-app constitution blocks, pointer lines instead of duplicated tables) lives in `@devflow/skills/devflow-setup/SKILL.md` prose, per the hook-vs-skill business-logic rule above.
